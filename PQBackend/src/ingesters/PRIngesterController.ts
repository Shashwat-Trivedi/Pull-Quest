...
import MergedPR from "../model/MergedPrs"; // Import the MergedPR model
import mongoose from "mongoose";

export const ingestMergedPR: RequestHandler = async (req, res) => {
  try {
    const {
      userId,
      githubUsername,
    ...
    const existingPR = await MergedPR.findOne({ id: ghPR.id });
    if (existingPR) {
      console.log(`Merged PR ${ghPR.id} already exists, updating...`);
      
      // Update existing merged PR with new data
      const updatedPR = await MergedPR.findOneAndUpdate(
        { id: ghPR.id },
        {
    ...
    const newMergedPR = new MergedPR({
      // Core PR fields
      id: ghPR.id,
      number: ghPR.number,
      ...
      assignees: (ghPR.assignees || []).map((u: any) => ({
        id: u.id,
        login: u.login,
      ...
      // Labels
      labels: (ghPR.labels || []).map((lbl: any) => ({
        id: lbl?.id || 0,
        name: lbl?.name || "",
      ...
      // Review information
      reviewRequests: (ghPR.requested_reviewers || []).map((reviewer: any) => ({
        user: {
          id: reviewer.id,
      ...
      reviews: prReviews.map((review: any) => ({
        id: review.id,
        user: {
          id: review.user.id,
      ...
      // Commit information
      commitsList: prCommits.map((commit: any) => ({
        sha: commit.sha,
