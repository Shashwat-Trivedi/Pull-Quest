...
const HederaGithubController_1 = require("../controllers/HederaGithubController");
const router = (0, express_1.Router)();
router.post('/ai-pr-review', async (req, res) => {
    await HederaGithubController_1.hederaGithubService.handleAIPRReview(req, res);
});
router.post('/mint-contributor-nft', async (req, res) => {
    await HederaGithubController_1.hederaGithubService.mintContributorNFT(req, res);
});
router.post('/create-bounty', async (req, res) => {
    await HederaGithubController_1.hederaGithubService.createBountyWithAI(req, res);
});
/**
...
router.get('/contributor-reputation/:username', async (req, res) => {
    try {
        ...
        const contributorStats = await HederaGithubController_1.hederaGithubService.fetchContributorStats(username);
        const reputationAssessment = await HederaGithubController_1.hederaGithubService.assessContributorReputation(contributorStats);
        res.json({
            success: true,
            username,
...
router.post('/federated-learning/start', async (req, res) => {
    try {
        ...
        const sessionRecord = await HederaGithubController_1.hederaGithubService.recordSessionOnHedera({
            sessionId,
            modelType,
...
//# sourceMappingURL=hederaRoutes.js.map