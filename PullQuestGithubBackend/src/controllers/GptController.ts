...
export const fullReview: RequestHandler = async (req, res) => {

  const { owner, repo, prNumber, requester } = req.body as FullReviewRequest;
...
async function generateFullAIReview(
  openai: OpenAI,
  prData: {
    title: string;
    description: string;
...
- Use technical precision in your analysis`;
  ...
  try {
    ...
    let bonusTotal = 0;
    if (result.bonusPoints.isBountyBacked) bonusTotal += 10;
    if (result.bonusPoints.mergedWithin48hrs) bonusTotal += 5;
    if (result.bonusPoints.contributorReviewedOthers) bonusTotal += 5;
    if (result.bonusPoints.addedMeaningfulTests) bonusTotal += 10;
...
async function postFullReviewComment(
  owner: string,
  repo: string,
  prNumber: number,
  ...
  const ratingDisplay = (rating: number) => '⭐'.repeat(rating) + '☆'.repeat(5 - rating);
