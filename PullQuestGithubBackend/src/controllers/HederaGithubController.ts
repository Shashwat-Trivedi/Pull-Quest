// src/controllers/HederaGithubController.ts
...
const PRReviewSchema = z.object({
  ...
  tokenReward: z.number().min(0).max(1000),
  summary: z.string(),
...
const ContributorReputationSchema = z.object({
  reputationScore: z.number().min(0).max(100),
  contributions: z.number(),
  qualityAverage: z.number(),
  trustLevel: z.enum(["BRONZE", "SILVER", "GOLD", "PLATINUM"]),
  recommendedRole: z.string(),
});

// Structured output parsers
const prReviewParser = StructuredOutputParser.fromZodSchema(PRReviewSchema);
const reputationParser = StructuredOutputParser.fromZodSchema(ContributorReputationSchema);

console.log("📋 Zod schemas and parsers initialized successfully");

interface HederaGithubPRRequest {
  owner: string;
  repo: string;
  prNumber: number;
  author: string;
  action?: 'review' | 'reward' | 'reputation';
}
...
export class HederaGithubService {
  
private async getHederaAgent() {
  try {
    // For now, return a mock agent to test the rest of the system
    return {
      submitMessageToTopic: async (topicId: string, message: string) => {
      ...
      createToken: async (name: string, symbol: string, amount: number, decimals: number, treasury: string) => {
      ...
      transferToken: async (tokenId: string, recipient: string, amount: number) => {
        console.log(`Mock: Transferring ${amount} of token ${tokenId} to ${recipient}`);
        return {
          transactionId: `mock-tx-${Date.now()}`
        };
      },
      mintNFT: async (tokenId: string, metadata: string, recipient: string) => {
   ...
   */
  async handleAIPRReview(req: Request, res: Response): Promise<void> {
    ...
    try {
      const { owner, repo, prNumber, author, githubToken } = req.body as HederaGithubPRRequest & { githubToken?: string };
   ...
   */
  async mintContributorNFT(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const nftResult = await hederaAgent.mintNFT(
        process.env.HEDERA_TOKEN_ID!, // Your contributor NFT collection
        JSON.stringify(nftMetadata),
        process.env.HEDERA_ACCOUNT_ID! // Recipient
      );
  ...
  async createBountyWithAI(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const hederaAgent = await this.getHederaAgent();
      const bountyToken = await hederaAgent.createToken(
        `Bounty-${Date.now()}`,
        "BNTY",
        bountyAmount,
        18,
  ...
  public async fetchPRData(owner: string, repo: string, prNumber: number, githubToken?: string) {
  ...
  public async generateAIReview(prData: any) {
...
`);
  ...
  public async recordReviewOnHedera(review: any, prData: any) {
    ...
    try {
      ...
      const hederaAgent = await this.getHederaAgent();
      const transaction = await hederaAgent.submitMessageToTopic(
        process.env.HEDERA_TOPIC_ID!, // Your review audit topic
        message
      );
  ...
  public async rewardContributor(contributor: string, tokenAmount: number) {
    ...
    try {
      ...
      const hederaAgent = await this.getHederaAgent();
      const transfer = await hederaAgent.transferToken(
        process.env.HEDERA_REWARD_TOKEN_ID!, // Your reward token
  ...
  public async updateContributorReputation(githubUsername: string, qualityScore: number) {
    ...
    try {
      ...
      const reputationRecord = await hederaAgent.submitMessageToTopic(
  ...
  public async assessContributorReputation(contributorStats: any) {
...
`);
  ...
  public async generateNFTMetadata(username: string, reputation: any): Promise<ContributorNFTMetadata> {
    ...
    try {
      const metadata: ContributorNFTMetadata = {
  ...
  public async postAICommentToGithub(owner: string, repo: string, prNumber: number, review: any, hederaTransactionId: string, githubToken?: string) {
...
## 💡 Recommendations
${review.recommendations.map((rec: string) => `- ${rec}`).join('\n')}
...
`;
  ...
  public getScoreEmoji(score: number): string {
  ...
  public getRiskEmoji(risk: string): string {
  ...
  public calculateReputationScore(history: any, newScore: number): number {
  ...
  public async getHederaAccountForGithubUser(githubUsername: string): Promise<string | null> {
  ...
  public async fetchContributorHistory(githubUsername: string): Promise<any> {
    console.log(`📖 Fetching contributor history for: ${githubUsername}`);
  ...
  public async fetchContributorStats(githubUsername: string): Promise<any> {
  ...
  public async analyzeIssueComplexity(issueUrl: string): Promise<any> {
  ...
  public async deployBountyContract(bountyAmount: number, requirements: string[], analysis: any): Promise<any> {
  ...
  public async recordSessionOnHedera(sessionData: any) {
    ...
    try {
      ...
      const transaction = await hederaAgent.submitMessageToTopic(
        process.env.HEDERA_TOPIC_ID!, // Your session audit topic
        message
      );
...
export const hederaGithubService = new HederaGithubService();
